...
public abstract class JpaRepoImpl<T> implements JpaRepo<T> {
    ...
    public static void main(String[] args) {
        JpaRepo<Product> productJpaRepo = new ProductService();
        SearchEngine searchEngine = new SearchEngine();
        searchEngine.setColumnList(Arrays.asList("name", "id"));
        searchEngine.setOperatorList(Arrays.asList(StringSql.Operator.LIKE, StringSql.Operator.EQUAL));
        searchEngine.setValueList(Arrays.asList("a", "3"));
        searchEngine.setConditionList(Collections.singletonList(StringSql.ExtendCondition.OR));
        List<Product> products = productJpaRepo.search(searchEngine);
    ...
    public void deleteById(long id) {
        String sql = SqlBuilder.deleteById(tableName, idColumn.columnName(), id);
        System.out.println(sql);
        Connection connection = DataSource.getConn();
        PreparedStatement preStmt;
        try {
